...
from othello.evaluation_functions import (
    evaluate_by_material,
    evaluate_by_mobility,
    evaluate_by_mobility_and_material,
)
    ...
    for d in range(2, 6):
        for eval_func in [
            ...
            runner = Runner(
                ...
                {
                    OthelloGame.Player.PLAYER1: AIPlayer(
                        CuttingoffSearch(evaluation=eval_func, depth=d)
                    ),
                    OthelloGame.Player.PLAYER2: AIPlayer(
                        CuttingoffSearch(evaluation=eval_func, depth=d)
                    ),
                },
            )
